...
export class PilotService {
  ...
  addPilot$(pilot: Pilot): Observable<any> {
    return this.httpClient.post<Pilot>('/api/pilotes', pilot).pipe(
  ...
  getPilot$(id): Observable<Pilot> {
    return this.httpClient.get<Pilot>(`/api/pilotes/${id}`).pipe(
  ...
  deletePilot$(id): Observable<any> {
    return this.httpClient.delete(`/api/pilotes/${id}`).pipe(
        tap(_ => console.log('Pilot Get One')),
        catchError(this.handleError<Pilot>('Get One Pilot', null))
    );
  }
  ...
  editPilot$(Pilot:Pilot): Observable<any> {
    return this.httpClient.put<any>(`/api/pilotes/${Pilot.id}`,Pilot).pipe(
